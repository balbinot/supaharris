...
class Command(PrepareSupaHarrisDatabaseMixin, BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not sigma_0:
            ...
            sigma_0.description = (
                "Central surface density of stars at the cluster center"
            )
        ...
        for i, cluster in enumerate(data):
            ...
            cluster, created = AstroObject.objects.get_or_create(name=name)
            ...
            for k, v in parameter_map.items():
                ...
                if not parameter:
                    print(
                        "ERROR: Parameter instance unknown, please add or correct {0}".format(
                            k
                        )
                    )
                    import sys
                ...
                print(
                    "  {0} (vandenBerg) --> {1} (SupaHarris)".format(v, parameter.name)
                )
                value = data[v][i]
                if v in ["Age"]:
                ...
                observation, created = Observation.objects.get_or_create(
                    astro_object=cluster,
                    reference=vandenberg_2013,
                    parameter=parameter,
                    value=value,
                    sigma_up=sigma_up,
