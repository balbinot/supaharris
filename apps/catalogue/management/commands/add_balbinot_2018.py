...
class Command(PrepareSupaHarrisDatabaseMixin, BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        mu, created = Parameter.objects.get_or_create(
            name="mu", description="Remaining mass fraction of cluster", scale=1.0
        )
        if not created:
        ...
        (has_pm,) = numpy.where(
            numpy.isfinite(database["mu_alpha"]) & numpy.isfinite(database["mu_delta"])
        )
        Nentries = len(has_pm)
        for i, entry in enumerate(database[has_pm]):
            ...
            if "," in refs:
                self.logger.warning(
                    "  WARNING: {0} has multiple pm refs\n    {1}".format(
                        gc_name,
                        "\n    ".join(
                            pm_reference_map[int(r)] for r in refs.split(",")
                        ),
                    )
                )
                ...
                self.logger.warning(
                    "  WARNING: {0} has no Refs. Using BG18".format(gc_name)
                )
                first_ref = BG18
            ...
            for param in ["mu_alpha", "mu_delta"]:
                obs, created = Observation.objects.get_or_create(
                    ...
                    value=entry[param],
                    sigma_up=entry[param + "_err"],
                    sigma_down=entry[param + "_err"],
                )
                if not created:
